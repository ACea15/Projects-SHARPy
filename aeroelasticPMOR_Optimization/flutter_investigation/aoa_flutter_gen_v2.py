# Code to obtain flutter speed for a single combination of aoa, ar and taper
...
foldername = 'aoa_flutter_investigation'
dirname = os.getcwd()
...
aoa_deg = np.array([0.,0.3,0.6,0.9,1.2,1.5,1.8,2.1,2.4,2.7])
ar = np.array([32.,])
taper = np.array([1.,])
...
for mi in range(num_models):
    ...
    if os.path.exists(file2check):
        ...
        if os.path.exists(errorpath):
            u_flutter[mi] = 0.      # Set the flutter speed to zero
        else:
