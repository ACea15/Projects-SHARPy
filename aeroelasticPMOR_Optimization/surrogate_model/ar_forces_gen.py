# Code to obtain lift, drag and moment for varying AR at a constant Uinf of
# 10m/s, Sref of 32m^2, taper ratio = 1, dhdrlSpanFraction = 0.25, twist=0,
# payload mass position=50kg, position=0 node of the fuselage
# Date start: 15/04/22
...
for i in range(len(AR)):
    ...
    if os.path.exists(file2check):
        ...
        g1 = gm.Model('sharpy', ['sharpy'],
                      ...
                      components_dict=comp_settings(wing_length[i],
                                                    winglt_length[i],
                                                    [chord[i],chord[i]],
                                                    [chord[i],chord[i]],
