# Code to obtain wing deformations i.e deflection at the tip and twist at some
# other station by varying velocity
# Date of start: 28/03/22
...
for i in range(len(u_inf)):
    ...
    file2write = targetpath + '/vel_%s' %int(u_inf[i]) + '/vel_%s' %int(u_inf[i])
    mi = 0
    ...
    os.chdir('./velocity_wingDef/vel_'+'%s' %int(u_inf[i]))
    data = sharpy.sharpy_main.main(['', 'vel_%s.sharpy' %int(u_inf[i])])
...
# Write to a csv file
data_pandas.to_csv('vel_wing_def.csv')
