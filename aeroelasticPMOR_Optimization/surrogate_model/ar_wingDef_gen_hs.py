...
# Using a horseshoe solution
# Date of start: 31/03/22
...
foldername = 'aspectRatio_wingDef'
dirname = os.getcwd()
...
for i in range(len(AR)):
    g1 = gm.Model('sharpy', ['sharpy'],
                  model_dict=model_settings('arhs_%s' %AR[i]),
    ...
    folder2write = targetpath + '/arhs_%s' % int(AR[i])
    file2write = targetpath + '/arhs_%s' % int(AR[i]) + '/arhs_%s' % int(AR[i])
    mi = 0
    ...
    os.chdir('./aspectRatio_wingDef/arhs_'+'%s' %int(AR[i]))
    data = sharpy.sharpy_main.main(['', 'arhs_%s.sharpy' %AR[i]])
...
# Write to a csv file
data_pandas.to_csv('arhs_wing_def.csv')
